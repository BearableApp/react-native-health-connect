...
interface BloodPressureRecordResult extends Omit<BloodPressureRecord, 'systolic' | 'diastolic'> {
    systolic: PressureResult;
    diastolic: PressureResult;
}
interface BodyTemperatureRecordResult extends Replace<BodyTemperatureRecord, 'temperature', {
    inFahrenheit: number;
    inCelsius: number;
}> {
}
...
interface HeartRateVariabilityRmssdRecordResult extends HeartRateVariabilityRmssdRecord {
}
interface WeightRecordResult extends Replace<WeightRecord, 'weight', MassResult> {
}
interface SleepSessionRecordResult extends SleepSessionRecord {
}
type HealthConnectRecordResult = BloodPressureRecordResult | BodyTemperatureRecordResult | HeartRateRecordResult | RestingHeartRateRecordResult | StepsRecordResult | HeartRateVariabilityRmssdRecordResult | WeightRecordResult | SleepSessionRecordResult;
export type RecordResult<T extends RecordType> = Omit<Extract<HealthConnectRecordResult, {
    recordType: T;
}>, 'recordType'>;
